...
enum Phase {
    ...
    Idle,
    /// We have MIN_FINGERS slots but haven't exceeded the movement threshold yet.
    Recognizing,
...
impl TouchSwipeGestureTracker {
    ...
    pub fn touch_down(
        ...
        if self.phase == Phase::Idle && self.slots.len() >= MIN_FINGERS {
            trace!(
    ...
    pub fn touch_up(
        ...
        match self.phase {
            ...
            Phase::Recognizing => {
                ...
                if self.slots.len() < MIN_FINGERS {
                    trace!("touch_swipe: dropped below {} fingers during recognition, cancelling", MIN_FINGERS);
                    self.reset_internal();
...
mod tests {
    ...
    fn no_gesture_with_fewer_than_three_fingers() {
        let mut tracker = TouchSwipeGestureTracker::new();
        ...
        assert!(tracker.touch_motion(slot(0), pt(200.0, 100.0), ts(10)).is_none());
        assert!(tracker.touch_motion(slot(1), pt(210.0, 100.0), ts(10)).is_none());
    ...
    fn gesture_begins_after_threshold_with_three_fingers() {
        ...
        assert!(matches!(ev, Some(GestureEvent::Begin { direction: SwipeDirection::Horizontal, .. })));
        assert!(tracker.is_active());
    ...
    fn vertical_direction_detected() {
        ...
        assert!(matches!(ev, Some(GestureEvent::Begin { direction: SwipeDirection::Vertical, .. })));
    }
    ...
    fn updates_emit_centroid_delta() {
        ...
        match ev {
            Some(GestureEvent::Update { delta_x, delta_y, .. }) => {
    ...
    fn begin_delta_reflects_cumulative_movement() {
        ...
        match ev {
            Some(GestureEvent::Begin { delta_x, delta_y, direction, .. }) => {
